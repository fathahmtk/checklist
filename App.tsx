...
const App: React.FC = () => {
    ...
    // --- Styles ---
    const cardStyles = "mt-8 bg-slate-800/50 rounded-2xl shadow-2xl backdrop-blur-md border border-slate-700 overflow-hidden";
    const buttonStyles = "px-4 py-2 text-sm font-semibold rounded-lg transition-colors duration-200";
    const primaryButton = `${buttonStyles} bg-sky-600 hover:bg-sky-500 text-white`;
    const secondaryButton = `${buttonStyles} bg-slate-600 hover:bg-slate-500 text-slate-200`;
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <main className={cardStyles}>
                    <div className="p-6 border-b border-slate-700">
                        <h2 className="text-xl font-bold text-slate-200 mb-2">Project Progress</h2>
                        <p className="text-slate-400 mb-4">{completedTasks} of {totalTasks} tasks completed</p>
                        <ProgressBar progress={progress} />
                    </div>
                     <div className="p-4 flex flex-wrap gap-2 border-b border-slate-700">
                ...
                <section className={cardStyles}>
                    <div className="p-6 border-b border-slate-700">
                         <h2 className="text-xl font-bold text-slate-200">Daily Process Log</h2>
                        <p className="text-slate-400 mt-1">Capture your daily progress, blockers, and next steps.</p>
                    </div>
                    <div className="p-4 flex flex-wrap gap-2 border-b border-slate-700">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-slate-300">
                            <thead className="text-xs text-sky-300 uppercase bg-slate-900/50">
                                <tr>
                                    <th scope="col" className="p-2 w-[150px]">Date</th>
                                    <th scope="col" className="p-2">Worked On</th>
                                    <th scope="col" className="p-2 w-[100px]">Hours</th>
                                    <th scope="col" className="p-2">Blockers / Issues</th>
                                    <th scope="col" className="p-2">Next Actions (Tomorrow)</th>
                                    <th scope="col" className="p-2 w-[80px] text-center">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {logs.map(log => (
                                    <tr key={log.id} className="border-b border-slate-700">
                                        ...
                                        <td className="p-2 text-center">
                                          <button onClick={() => handleDeleteLog(log.id)} className="px-2 py-1 rounded-md text-slate-300 hover:bg-red-500/50 hover:text-red-300 transition" aria-label="Delete log entry">✕</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
